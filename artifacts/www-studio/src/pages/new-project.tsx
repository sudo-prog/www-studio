...
import { WandSparkles, Link as LinkIcon, Loader2, Sparkles, ImageUp, Upload, X } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { cn } from "@/lib/utils";

type Tab = "clone" | "generate" | "screenshot";
...
export default function NewProject() {
  ...
  const handleScreenshot = (e: React.FormEvent) => {
    ...
    screenshotMutation.mutate({
      data: { image: imageData.base64, mimeType: imageData.mimeType, filename: imageData.filename },
    }, {
      onSuccess: (result) => {
        toast({ title: "Screenshot converted!" });
        setLocation(`/editor/${result.projectId}`);
      },
      ...
      <main className="flex-1 flex items-start justify-center p-6 pt-12">
        <div className="w-full max-w-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground">Clone a site, generate from a prompt, or convert a screenshot.</p>
          </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["stripe.com", "linear.app", "vercel.com", "apple.com"].map((u) => (
                      <button
                        ...
                        className="text-xs px-3 py-1 rounded-full border border-border/50 text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors"
                      >
                        {u}
                      </button>
                    ))}
          ...
          {activeTab === "generate" && (
            <Card className="border-muted bg-card/50 backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>
                  Describe your website idea and AI will generate a full landing page.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleGenerate} className="space-y-4">
                  <Textarea
                    placeholder="A dark SaaS landing page for a project management tool with pricing section and testimonials..."
                    value={prompt}
                    ...
                    <div className="space-y-1.5">
                      {PROMPT_EXAMPLES.map((ex) => (
                        <button
                          ...
                          className="w-full text-left text-xs px-3 py-2 rounded-lg border border-border/40 text-muted-foreground hover:text-foreground hover:border-primary/40 hover:bg-primary/5 transition-all"
                        >
                          {ex}
                        </button>
                      ))}
          ...
          {activeTab === "screenshot" && (
            <Card className="border-muted bg-card/50 backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>
                  Upload any UI screenshot or design mockup — AI converts it to Tailwind HTML.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {!imagePreview ? (
                    <div
                      ...
                      className={cn(
                        ...
                        dragOver ? "border-primary bg-primary/5" : "border-border/50 hover:border-primary/50 hover:bg-muted/30"
                      )}
                    >
