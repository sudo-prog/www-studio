                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5">
                      {PROMPT_EXAMPLES.map((ex) => (
                        <button key={ex} type="button" onClick={() => setPrompt(ex)} className="w-full text-left text-xs px-3 py-2 min-h-[48px] rounded-lg border border-border/40 text-muted-foreground hover:text-foreground hover:border-primary/40 hover:bg-primary/5 transition-all">{ex}</button>
                      ))}
                    </div>
                  </div>
                ...
                <form onSubmit={handleScreenshot} className="space-y-4">
                  {!imagePreview ? (
                    ...
                    <div className="relative rounded-xl overflow-hidden border border-border/50">
                      ...
                      <button type="button" onClick={() => { setImagePreview(null); setImageData(null); }} className="absolute top-2 right-2 min-w-[48px] min-h-[48px] rounded-full bg-background/80 backdrop-blur flex items-center justify-center hover:bg-background transition-colors"><X className="w-4 h-4" /></button>
                    </div>
                  )}
                ...
                <form onSubmit={handleFigmaImport} className="space-y-4">
                  {!figmaJson ? (
                    ...
                    <div className="relative rounded-xl border border-border/50 bg-muted/20 p-4 flex items-center gap-3">
                      ...
                      <button type="button" onClick={() => { setFigmaJson(null); setFigmaFilename(null); }} className="min-w-[48px] min-h-[48px] rounded-full bg-background/80 flex items-center justify-center hover:bg-background transition-colors shrink-0"><X className="w-4 h-4" /></button>
                    </div>
                  )}
