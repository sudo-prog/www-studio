...
type RightPanel = "properties" | "images";
    ...
    <div className="flex flex-col h-full">
      <form onSubmit={handleGenerate} className="p-3 border-b border-border/50 space-y-2">
        ...
        <Button type="submit" size="sm" className="w-full gap-1.5" disabled={generateImage.isPending || !imgPrompt.trim()}>
          {generateImage.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
          Generate
        </Button>
      </form>
...
function CanvasPreview({ projectId, deviceMode }: { projectId: string; deviceMode: DeviceMode }) {
  ...
  const previewUrl = `/api/projects/${projectId}/preview-html`;
...
export default function Editor() {
  ...
  // Parse component tree into layer items
  const layers = (() => {
      ...
      <header className="h-14 border-b border-border/50 bg-card/50 backdrop-blur flex items-center justify-between px-4 shrink-0 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {project.status === "published" && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-500 border border-green-500/20 shrink-0">
              Live
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button variant="outline" size="sm" className="gap-2 hidden md:flex">
            <Wand2 className="w-4 h-4" /> Variants
          </Button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-56 border-r border-border/50 bg-card/30 flex flex-col shrink-0">
          ...
          <ScrollArea className="flex-1 py-1">
            {layers.length > 0 ? (
              layers.map((layer, i) => (
                <LayerItem key={i} name={layer.name} depth={layer.depth} selected={i === 0} />
              ))
            ) : (
        ...
        <aside className="w-64 border-l border-border/50 bg-card/30 flex flex-col shrink-0">
          ...
          <div className="flex border-b border-border/50 shrink-0">
            {([
              { id: "properties" as RightPanel, icon: <ChevronDown className="w-3.5 h-3.5" />, label: "Properties" },
              { id: "images" as RightPanel, icon: <Image className="w-3.5 h-3.5" />, label: "AI Images" },
            ] as const).map((tab) => (
              <button
                key={tab.id}
                onClick={() => setRightPanel(tab.id)}
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 h-9 text-xs font-medium transition-colors border-b-2",
                  rightPanel === tab.id
              ...
              <div className="space-y-5">
                <div className="space-y-2.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      ...
                      <Input className="h-8 text-xs font-mono" defaultValue="24px" />
                    </div>
                    <div className="space-y-1">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input className="h-8 text-xs font-mono" defaultValue="1.5" />
                  </div>
                </div>
                <div className="w-full h-px bg-border/50" />
                <div className="space-y-2.5">
          ...
          {rightPanel === "images" && (
            <AIImagesPanel projectId={project.id} />
          )}
        </aside>
      </div>
    </div>
