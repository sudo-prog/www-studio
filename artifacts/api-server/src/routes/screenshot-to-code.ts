        ...
        children: [
          {
            ...
            children: [
              ...
              { type: "p", content: "Generated from uploaded screenshot. Add an OpenAI API key for precise code extraction.", className: "text-gray-400 text-xl mb-8" },
...
async function extractCodeFromImage(openai: OpenAI, base64Image: string, mimeType: string): Promise<string> {
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a web design AI that converts UI screenshots into JSON component trees.
...
Match the layout, spacing, colors, and typography from the screenshot as closely as possible using Tailwind CSS. Return ONLY the JSON.`,
      },
      {
        role: "user",
        content: [
          { type: "text", text: "Convert this UI screenshot into a JSON component tree:" },
...
router.post("/screenshot-to-code", async (req: Request, res: Response) => {
  ...
  if (openai) {
    try {
      componentTree = await extractCodeFromImage(openai, image, mimeType);
    } catch {
